...
export default class RestStorageService extends StorageService{
    ...
    async UpdateTeams(params = null){
        ...
        await fetch(`http://${this.host}/${this.apiName}/${this.ParamBuilder(params)}`)
            ...
            .then((resp)=> {
                ...
                //Get images
                this.model.teams.every((team)=>this.DownloadImage(team));
    ...
    //Get's logo from server
    async DownloadImage(team){
        //Make sure it doesn't have a path
        if (team.TeamLogo.includes("/")){
            team.TeamLogo= `http://${this.host}/`+team.TeamLogo;
            return true;
        }
        //Default logo (api is trimming url)
        if (team.TeamLogo.includes("default.png")){
            team.TeamLogo= `http://${this.host}/`+this.defaultTeamLogo;
            return true;
        }

        //Get url
        return await fetch(`http://${this.host}/${this.apiName}/logo/${team.TeamLogo}`).then((resp) => {
            //Check status code
            if (resp.statusCode === 204)
                team.TeamLogo = this.defaultTeamLogo;

            else
                team.TeamLogo = resp.url;
            return true
        }).catch((err)=>{
            throw (err);
        });
