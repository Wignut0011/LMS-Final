...
//Makes sure empty image submissions get the default logo and that upload is just the filename
Team.cleanImageURL = (team) =>{
    if (!team.TeamLogo || team.TeamLogo === DEFAULT_LOGO) { //Put logo default if url empty
        team.TeamLogo = DEFAULT_TEAM_LOGO;
        return;
    }
    //Take file name out
    let fileName = team.TeamLogo.split('/');
    team.TeamLogo = fileName[fileName.length-1];
}
