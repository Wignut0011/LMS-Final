...
const LOGO_PATH = "img/Team_img/";
const LOCAL_LOGO_PATH = "./img/Team_img/";
const DEFAULT_LOGO = "default.png";
const DEFAULT_TEAM_PATH = "img/Def_Team_img/"
const DEFAULT_TEAM_PATH_F = ".img/Def_Team_img/"
...
exports.uploadFile = async(req,res) => {
    ...
    let fileName = params.split(' ').join('_') + '.' + file.mimetype.split('/')[1];
    let path = LOGO_PATH + fileName;

    try {
        //Unlink if already exists, DON'T DELETE ORIGINAL TEAM LOGO OR DEFAULT LOGO
        if (fs.existsSync(path) && path !== LOGO_PATH + DEFAULT_LOGO && !path.startsWith(DEFAULT_TEAM_PATH) && !path.startsWith(DEFAULT_TEAM_PATH_F))
            Team.removeImg(path);

        //Upload
        Team.uploadImg(path, file, res, fileName);
    } catch (err) {
        throw err;
    }
}
...
exports.downloadFile = async(req, res)=> {
    ...
    if (!req.params.fileName) {
        throw Error.BadRequest('Missing File Name In Request');
    }

    //Send back default route
    if (req.params.fileName===DEFAULT_LOGO){
        res.status(204).send();
        return;
    }

    //Send image back, not default.
    await fs.readFile(LOGO_PATH+req.params.fileName, (err, resp)=>{
        if (err)
            throw Error.NotFound("Logo Not Found")
...
exports.del = async(req, res)=> {
    ...
    let path = LOGO_PATH + thisTeam[0].TeamLogo;
    if (path !== "public/" && path !== LOGO_PATH + DEFAULT_LOGO && !path.includes(DEFAULT_TEAM_PATH)) {
